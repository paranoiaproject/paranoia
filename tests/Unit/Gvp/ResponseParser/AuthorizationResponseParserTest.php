...
class AuthorizationResponseParserTest extends TestCase
{
    ...
    {
        $content = \file_get_contents(__DIR__ . '/../../../stub/gvp/response/authorization_successful.xml');
        ...
        $this->assertEquals('412290', $authorizationResponse->getAuthCode());
        $this->assertEquals('311710676052', $authorizationResponse->getTransactionId());
    }

    public function test_parse_with_failed_message()
    {
        $content = \file_get_contents(__DIR__ . '/../../../stub/gvp/response/authorization_failed.xml');
        /** @var ResponseInterface $responseMock */
        $responseMock = $this->getResponseMock($content);
