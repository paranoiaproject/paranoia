...
class Iyzico extends AdapterAbstract
{
    ...
    {
        $expireDate = $this->formatExpireDate($request->getExpireMonth(),
    ...
    protected function buildSaleRequest(Request $request)
    {
        ...
        $requestData = array_merge($base,
                                    array_merge($this->buildCard($request),
                                                array(
                                                    'amount'    => $amount,
                                                    'currency' => $currency,
    ...
    protected function buildPreAuthorizationRequest(Request $request)
    {
        ...
        $base = $this->buildBaseRequest($request, $this->getProviderTransactionType(self::TRANSACTION_TYPE_PREAUTHORIZATION));
        $amount      = $this->formatAmount($request->getAmount());
        $currency    = $this->formatCurrency($request->getCurrency());
        $requestData = array_merge($base,
                                    array_merge($this->buildCard($request),
    ...
    {
        $base = $this->buildBaseRequest($request, $this->getProviderTransactionType(self::TRANSACTION_TYPE_POSTAUTHORIZATION));
        $amount      = $this->formatAmount($request->getAmount());
        $currency    = $this->formatCurrency($request->getCurrency());
        $requestData = array_merge($base,
                                    array_merge($this->buildCard($request),
